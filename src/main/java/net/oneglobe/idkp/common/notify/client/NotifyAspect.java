 ...
 * methods which return an Object (element) and are annotated with NotifyChange.
 * If the aspect is triggered, it will publish a ChangeEvent containing the
 * element and change type using an ApplicationEventPublisher.
 *
...
public class NotifyAspect {
     ...
     * aspect to trigger for service methods returning an Object (element).
     *
     * @param notifyChange
     * @param notifyable
     */
    @AfterReturning(pointcut = "execution(public Object+ net.oneglobe.idkp.*.service.*.*(..)) && notifyChangePointcut() && @annotation(notifyChange)", returning = "notifyable")
    public void notify(NotifyChange notifyChange, Object notifyable) {
